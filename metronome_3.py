...
class Metro:
    def __init__(self):
        ...
        self.win.bind("<ButtonPress-1>", self.StartMove)  # 监听左键按下操作响应函数
        self.win.bind("<ButtonRelease-1>", self.StopMove)  # 监听左键松开操作响应函数
        self.win.bind("<B1-Motion>", self.OnMotion)  # 监听鼠标移动操作响应函数
        "其他"
    ...
    def OnMotion(self, event):
        if event.widget != self.scale:
            deltax = event.x - self.x
            deltay = event.y - self.y
            self.win.geometry("+%s+%s" % (self.win.winfo_x() + deltax, self.win.winfo_y() + deltay))
            self.win.update()
            # print(event.x, event.y, self.win.winfo_x(), self.win.winfo_y(), self.win.winfo_width(), self.win.winfo_height())
